  ...
  describe "POST /add_items" do
    context 'add a new product to cart' do
      context 'when cart do not exist' do
        it 'creates a new cart and and save id in session' do
          ...
          post carts_add_items_path, params: { product_id: product.id, quantity: 2 }
      ...
      context 'when product does not exist' do
        it 'return error message' do
          post carts_add_items_path, params: { product_id: 123, quantity: 2 }
    ...
    context 'when the product already is in the cart' do
      ...
      it 'updates the quantity of the existing item in the cart' do
        cart = create(:cart)
        product = create(:product, name: "Test Product", price: 10.0)
